...
class _AddPackageInputFieldState extends State<AddPackageInputField> {
  ...
  Widget packageSubTitle(Map<String, dynamic> package) => Text(
      "${package['quantity']} kg | ${currency.format(package['price'])} TZS");
        ...
        return Consumer<SaleProvider>(
          builder: (context, provider, child) {
            return Column(children: [
              ListTile(
                onTap: () => _openQrScanner(field, provider),
                ...
                trailing: widget.canEdit
                    ? IconButton(
                        ...
                        onPressed: () => _openQrScanner(field, provider),
                      )
                    : const Chip(
                        label: Text(
                          'SOLD',
  ...
  Future<void> _openQrScanner(
      FormFieldState field, SaleProvider provider) async {
    try {
      ...
      if (cameraPerm.isGranted) {
        ...
        if (cameraScanResult != null) {
          ...
          StockPackage? stockPackage =
              await provider.fetchPackage(fromScanner['qr']);
          ...
          if (stockPackage != null) {
            ...
            if (!exist) {
              currentPackages.add(stockPackage.toJson());
              field.didChange(currentPackages);
            }
          }
        }
